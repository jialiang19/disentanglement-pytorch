...
class PROVAE(BaseDisentangler):
    ...
    def __init__(self, args):
        ...
        self.model = PROVAEModel(new_encoder,
        ...
        # FactorVAE # FactorVAE portion has not being adapted
    ...
    def _kld_loss_fn(self, mu, logvar):
        if not self.controlled_capacity_increase:
            ...
            """
            capacity = torch.min(self.max_c, self.max_c *
                                 torch.tensor(self.iter) / self.iterations_c)
            kld_loss = (kl_divergence_mu0_var1(
                mu, logvar) - capacity).abs() * self.w_kld
        return kld_loss
    ...
    def loss_fn(self, input_losses, **kwargs):
        ...
        output_losses[c.RECON] = F.binary_cross_entropy(
        ...
        if c.DIPVAEI in self.loss_terms:
            ...
            output_losses['vae_dipi'] = dipvaei_loss_fn(
                self.w_dipvae, self.lambda_od, self.lambda_d, **kwargs)
        ...
        if c.DIPVAEII in self.loss_terms:
            ...
            output_losses['vae_dipii'] = dipvaeii_loss_fn(
                self.w_dipvae, self.lambda_od, self.lambda_d, **kwargs)
        ...
        if c.BetaTCVAE in self.loss_terms:
            ...
            output_losses['vae_betatc'] = betatcvae_loss_fn(
                self.w_tc, **kwargs)
        ...
        if c.INFOVAE in self.loss_terms:
            ...
            output_losses['vae_mmd'] = infovae_loss_fn(
                self.w_infovae, self.z_dim, self.device, **kwargs)
    ...
    def train(self):
        while not self.training_complete():
            ...
            for internal_iter, (x_true1, label1) in enumerate(self.data_loader):
                ...
                losses, params = self.vae_base(
                    losses, x_true1, x_true2, label1, label2)
                ...
                self.log_save(input_image=x_true1,
                              recon_image=params['x_recon'], loss=losses)
            # end of epoch
